...
class Directory:
    ...
    directories: MutableMapping['str', 'Directory'] = dataclasses.field(default_factory=dict)
    ...
    def total_size(self):
        ...
        self._total_size = 0
        self._total_size += sum(directory.total_size for directory in self.directories.values())
...
def solution(level):
    ...
    for line in utils.input_reader():
        if ls_mod:
            if line.startswith('$'):
                ...
                assert match
                assert cur
                cur.files[match['name']] = File(name=match['name'], size=int(match['size']))
                continue
    ...
    if level == 1:
        ...
        def collect(pos: Directory) -> List[int]:
            ...
            answer.extend(item for directory in pos.directories.values() for item in collect(directory))
            return answer
